  ...
  <div>
    ...
    <div class="divide-y divide-accent/20">
      <div
        ...
        :key="msg.id"
        :class="[
          'py-5 cursor-pointer hover:bg-surface/50 px-3 rounded-xl transition-colors',
          !msg.read && 'bg-surface/30',
        ]"
        @click="markRead(msg)"
      >
    ...
    <p v-if="messages.length === 0" class="text-center font-body text-primary/30 py-16">
      Aucun message.
    </p>
  </div>
</template>
...
const formatDate = (ts: Msg['createdAt']): string => {
  if (!ts) return ''
  const d = isTimestamp(ts) ? ts.toDate() : new Date(ts as unknown as string | number | Date)
